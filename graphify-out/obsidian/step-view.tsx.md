---
source_file: "components/views/step-view.tsx"
type: "code"
community: "Community None"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_None
---

# step-view.tsx

## Connections
- [[ServiceIcon()]] - `imports` [EXTRACTED]
- [[chevronClip()]] - `contains` [EXTRACTED]
- [[cn()_10]] - `imports` [EXTRACTED]
- [[cn()_6]] - `contains` [EXTRACTED]
- [[defaultStepId()]] - `contains` [EXTRACTED]
- [[flow-view.tsx]] - `imports_from` [EXTRACTED]
- [[getStepDetails()]] - `imports` [EXTRACTED]
- [[relativeFromNow()]] - `imports` [EXTRACTED]
- [[section-label.tsx]] - `imports_from` [EXTRACTED]
- [[service-icon.tsx]] - `imports_from` [EXTRACTED]
- [[serviceLabel()]] - `imports` [EXTRACTED]
- [[shortDateTime()_1]] - `imports` [EXTRACTED]
- [[step-details.ts]] - `imports_from` [EXTRACTED]
- [[stepTone()]] - `contains` [EXTRACTED]
- [[time.ts]] - `imports_from` [EXTRACTED]
- [[utils.ts]] - `imports_from` [EXTRACTED]
- [[workflows.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_None